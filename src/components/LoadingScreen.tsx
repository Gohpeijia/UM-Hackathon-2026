...
interface Person {
  id: string;
  status: 'queued' | 'moving' | 'deciding' | 'accepted' | 'rejected';
}

export default function LoadingScreen({ onComplete }: { onComplete: () => void }) {
  const [progress, setProgress] = useState(0);
  const [queue, setQueue] = useState<Person[]>(Array.from({ length: 5 }, (_, i) => ({ id: `p-${i}`, status: 'queued' })));
    ...
    setQueue(prev => {
      ...
      setTimeout(() => {
        setQueue(q => {
          ...
          const p: Person = { ...updated[0], status: 'deciding' };
          updated[0] = p;
          return updated;
        });
        ...
        setTimeout(() => {
          ...
          setQueue(q => {
            const updated = [...q];
            if (!updated[0]) return updated;
            const status: Person['status'] = isAccepted ? 'accepted' : 'rejected';
        ...
        <div className="glass-card">
          ...
          <div style={{ display: 'flex', alignItems: 'center', zIndex: 10, width: '100%', position: 'relative', height: '100%' }}>
            ...
            <div style={{ position: 'absolute', left: '45%', top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', zIndex: 20 }}>
              <div style={{ position: 'relative', width: '7rem', height: '7rem', display: 'flex', alignItems: 'center', justifyCenter: 'center' }}>
                <div 
                  style={{ 
                    position: 'absolute', 
                    inset: 0, 
                ...
                <div style={{ position: 'absolute', bottom: '-3rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
